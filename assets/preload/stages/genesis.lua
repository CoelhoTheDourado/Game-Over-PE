...
function onCreatePost()
	for b = 0, getProperty('unspawnNotes.length') - 1 do
		...
		if getPropertyFromGroup('unspawnNotes', b, 'noteType') ~= 'ring' then
			setPropertyFromGroup('unspawnNotes', b, 'texture', 'genesis/ui/notes')
		end
	end
	...
	makeLuaSprite('goal', 'genesis/bg', -40*4, -40*4+(24*4))
	...
	---/////OBJECTS MAKING////---
		makeAnimatedLuaSprite('pipe1', 'pcport/pipebreak', 0, -18*6)
		...
		makeAnimatedLuaSprite('pipe2', 'pcport/pipebreak', 0, -18*6)
		...
		makeAnimatedLuaSprite('emptyB4', 'pcport/emptybreak', 0, -18*6)
	...
	addAnimationByPrefix('mxLegs', 'idle', 'legs', 50, true)
	...
	setProperty('dad.x', 130*4)
	setProperty('dad.y', 54*4)
	
	makeLuaSprite('mxPoint', 'pcport/legs', 74*6, 8.95*6)
	makeLuaSprite('bfPoint', 'pcport/legs', 74*6, 8.95*6)
	
	makeLuaText('ringsShadow', '0', 0, 30*6-3.3, 28*6-2)
	setProperty('ringsShadow.borderSize', 0)
	setProperty('ringsShadow.antialiasing', false)
	setTextFont('ringsShadow', 'sonic.ttf')
	setTextSize('ringsShadow', 8*6, 8*6)
	setScrollFactor('ringsShadow', 0, 0)
	setObjectCamera('ringsShadow', 'game')
	setTextColor('ringsShadow', '000000')
	addLuaText('ringsShadow', true)
	
	makeLuaText('ringCount', '0', 0, getProperty('ringsShadow.x'), getProperty('ringsShadow.y')-(1*6))
	setProperty('ringCount.borderSize', 0)
	setProperty('ringCount.antialiasing', false)
	setTextFont('ringCount', 'sonic.ttf')
	setTextSize('ringCount', 8*6, 8*6)
	setScrollFactor('ringCount', 0, 0)
	setObjectCamera('ringCount', 'game')
	addLuaText('ringCount', true)
	
	makeLuaText('ringDisplayS', 'RINGS', 0, 5*6-3.3, 28*6-2)
	setProperty('ringDisplayS.borderSize', 0)
	setProperty('ringDisplayS.antialiasing', false)
	setTextFont('ringDisplayS', 'sonic.ttf')
	setTextSize('ringDisplayS', 8*6, 8*6)
	setScrollFactor('ringDisplayS', 0, 0)
	setObjectCamera('ringDisplayS', 'game')
	setTextColor('ringDisplayS', '000000')
	addLuaText('ringDisplayS', true)
	
	makeLuaText('ringDisplay', 'RINGS', 0, getProperty('ringDisplayS.x'), getProperty('ringDisplayS.y')-(1*6))
	setProperty('ringDisplay.borderSize', 0)
	setProperty('ringDisplay.antialiasing', false)
	setTextFont('ringDisplay', 'sonic.ttf')
	setTextSize('ringDisplay', 8*6, 8*6)
	setScrollFactor('ringDisplay', 0, 0)
	setObjectCamera('ringDisplay', 'game')
	setTextColor('ringDisplay', 'ECEF02')
	addLuaText('ringDisplay', true)
end
...
function goodNoteHit(id, noteData, noteType, isSustainNote)
	if noteType == 'ring' and not isSustainNote then
		ringCount = ringCount + 1
		setTextString('ringsShadow', ringCount)
		setTextString('ringCount', ringCount)
		playSound('ring-gain', 0.25)
	end
end

function noteMiss(id, direction, noteType, isSustainNote)
	if dmgCooldown <= 0 and noteType ~= 'ring' then
		dmgCooldown = 2.5
		if ringCount <= 0 then
		...
		local ringsFall = ringCount
		if ringsFall > 15 then
			ringsFall = 15
		end
		
		ringCount = ringCount - 15
		if ringCount <= 0 then
			ringCount = 0
		end
		setTextString('ringsShadow', ringCount)
		...
		for i=1, ringsFall do
			ringIndex = tostring(i)
			ringName = 'ring'..ringIndex
			local yOfs = math.random(-4, 4)
			local xOfs = math.random(5, 10)
			if i % 2 == 0 then
				xOfs = xOfs * -1
			end
				
			makeAnimatedLuaSprite(ringName, 'genesis/ringspin', getMidpointX('boyfriend') - 80, getMidpointY('boyfriend') - 80 + (20 * yOfs))
			addAnimationByPrefix(ringName, 'spin', 'ring', 24, true)
			objectPlayAnimation(ringName, 'spin', true)
			scaleObject(ringName, 4, 4)
			updateHitbox(ringName)
			setProperty(ringName..'.antialiasing', false)
			addLuaSprite(ringName, true)
			ringsArray[i] = getProperty(ringName)
			
			makeLuaSprite(ringName..'Pos', nil, getMidpointX('boyfriend') - 110, getMidpointY('boyfriend') - 80 + (20 * yOfs))
			ringsPosArray[i] = getProperty(ringName..'Pos')
			
			doTweenX('moveHorizontally'..ringIndex, ringName..'Pos', (getProperty(ringName..'Pos.x') + (xOfs * 45)), 0.8, 'linear')
			doTweenY('moveUp'..ringIndex, ringName..'Pos', (getProperty(ringName..'Pos.y') - 80), 0.25, 'sineOut')
		end
		playSound('ring-loss', 0.25)
	end
end
...
function onUpdate(elapsed)
	...
	if ringCount <= 0 then
		if timer <= 0 then
			...
			if not red then
				...
				setTextColor('ringDisplay', 'F60006')
			end
		else
			timer = timer - elapsed
		end
	...
	for i,ring in pairs(ringsArray) do
		...
		setProperty(ring..'.y', math.round(getProperty(ringsPosArray[i]..'.y')/4)*4)
		setProperty(ring..'.x', math.round(getProperty(ringsPosArray[i]..'.x')/4)*4)
	end
	
	if flashingLights then
		if getProperty('cpuControlled') and not stopFlicker then
			local hexColr = ''
			...
			--doTweenColor('starFlicker', 'boyfriend', hexColr, 0.01, 'linear')
			--doTweenColor('starFlickerLegs', 'bfLegs', hexColr, 0.01, 'linear')
	...
	if isChase then
		...
		if not pipeEnd then
			...
			for i=-1760*6, -178*6 do
				if getProperty('loop.x') == i and not jumpTriggers[4] then
					jumpTriggers[4] = true
					doTweenY('jumpBF', 'bfPoint', bfLegsHighY+(8*6), 0.5, 'sineOut')
					objectPlayAnimation('bfLegs', 'jump'..bfLegsPrefix, true)
					setProperty('boyfriend.idleSuffix', '-j')
					if getProperty('boyfriend.animation.curAnim.name') == 'idle' then
						characterPlayAnim('boyfriend', 'idle-j', true)
	...
	if curBeat == 0 then
		setPropertyFromGroup('playerStrums', 0, 'x', math.round((defaultOpponentStrumX0+80)/4)*4)

		setPropertyFromGroup('playerStrums', 1, 'x', math.round((defaultOpponentStrumX1+80)/4)*4)

		setPropertyFromGroup('playerStrums', 2, 'x', math.round((defaultOpponentStrumX2+80)/4)*4)

		setPropertyFromGroup('playerStrums', 3, 'x', math.round((defaultOpponentStrumX3+80)/4)*4)


		setPropertyFromGroup('opponentStrums', 0, 'x', math.round((defaultPlayerStrumX0-60)/4)*4)

		setPropertyFromGroup('opponentStrums', 1, 'x', math.round((defaultPlayerStrumX1-60)/4)*4)

		setPropertyFromGroup('opponentStrums', 2, 'x', math.round((defaultPlayerStrumX2-60)/4)*4)

		setPropertyFromGroup('opponentStrums', 3, 'x', math.round((defaultPlayerStrumX3-60)/4)*4)
		
		--y
		setPropertyFromGroup('playerStrums', 0, 'y', math.round((defaultOpponentStrumY0-30)/4)*4)

		setPropertyFromGroup('playerStrums', 1, 'y', math.round((defaultOpponentStrumY1-30)/4)*4)

		setPropertyFromGroup('playerStrums', 2, 'y', math.round((defaultOpponentStrumY2-30)/4)*4)

		setPropertyFromGroup('playerStrums', 3, 'y', math.round((defaultOpponentStrumY3-30)/4)*4)


		setPropertyFromGroup('opponentStrums', 0, 'y', math.round((defaultPlayerStrumY0-30)/4)*4)

		setPropertyFromGroup('opponentStrums', 1, 'y', math.round((defaultPlayerStrumY1-30)/4)*4)

		setPropertyFromGroup('opponentStrums', 2, 'y', math.round((defaultPlayerStrumY2-30)/4)*4)

		setPropertyFromGroup('opponentStrums', 3, 'y', math.round((defaultPlayerStrumY3-30)/4)*4)
	...
	for i,ring in pairs(ringsArray) do
		if ring ~= nil then
			if tag == 'moveUp'..i then
				doTweenY('moveDown'..i, ringsPosArray[i], (getProperty(ringsPosArray[i]..'.y') + 800), 0.8, 'sineIn')
			end
			
			if tag == 'moveDown'..i then
				removeLuaSprite(ringsPosArray[i])
			end
		end
	end
...
function onMoveCamera(focus)
	...
	setProperty('camGame.zoom', 1.001)
	setProperty('camGame.x', 28*6)
	setProperty('camGame.y', 23*6)
	setProperty('camHUD.zoom', 1.001)
...
function onTimerCompleted(tag, loops, loopsLeft)
	...
	if tag == 'countdown1' then
		...
		for i=0, getProperty('opponentStrums.length')-1 do
			...
			setPropertyFromGroup('opponentStrums', i, 'texture', 'genesis/ui/notes')
			setPropertyFromGroup('playerStrums', i, 'texture', 'genesis/ui/notes')
		end
	end
